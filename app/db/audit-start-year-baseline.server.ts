...
import { type FourEyesStatus, LEGACY_STATUSES_SQL } from '~/lib/four-eyes-status'
...
async function getGroupRepoScope(client: PoolClient, appIds: number[]): Promise<RepoScope | null> {
  ...
  const row = rows[0]
  if (!row) return null
  return { owner: row.github_owner, repo: row.github_repo_name }
}
...
export interface AuditStartYearChangeResult {
  ...
  demotedDeploymentId: number | null
}
...
function appendMarkerRepoScope(params: unknown[], repoScope: RepoScope | null): string {
  ...
  return ` AND (
    ...
    OR d.detected_github_owner IS NULL
    OR d.detected_github_repo_name IS NULL
  )`
}
...
async function recomputeBaseline(
...
): Promise<{ promotedDeploymentId: number | null; demotedDeploymentId: number | null }> {
  ...
  const oldMarker = markerRows[0] ?? null
  ...
  if (plan.demote) {
    const { rowCount } = await client.query(
      ...
      [plan.demote.toStatus, plan.demote.id, plan.demote.fromStatus],
    )
    if (rowCount === 0) {
      throw new Error(
        `Deployment ${plan.demote.id} four_eyes_status changed concurrently (expected ${plan.demote.fromStatus}); aborting baseline recompute`,
      )
    }
    await insertStatusHistory(
      client,
      plan.demote.id,
      plan.demote.fromStatus,
      plan.demote.toStatus,
      adminNavIdent,
      previousAuditStartYearByAppId.get(oldMarker?.monitored_app_id ?? -1) ?? null,
      newAuditStartYear,
    )
  }
  ...
  return {
    ...
    demotedDeploymentId: plan.demote?.id ?? null,
  }
}
...
export async function applyAuditStartYearChange(
  ...
  return withTransaction(async (client) => {
    ...
    const repoScope = await getGroupRepoScope(client, targetAppIds)

    const recomputeScopeAppIds = !repoScope && targetAppIds.length > 1 ? [appId] : targetAppIds

    const { promotedDeploymentId, demotedDeploymentId } = await recomputeBaseline(
      client,
