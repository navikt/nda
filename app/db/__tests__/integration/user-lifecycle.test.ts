...
describe('createUser — GitHub-løs bruker', () => {
  beforeEach(async () => {
  ...
  it('gjenfinner brukeren via getUserMappingByNavIdent', async () => {
    await createUser({ navIdent: 'Z990001', displayName: 'Stille Skog' })
  ...
  it('er idempotent — oppdaterer eksisterende bruker ved konflikt', async () => {
    await createUser({ navIdent: 'Z990001', displayName: 'Stille Skog' })
    const updated = await createUser({ navIdent: 'Z990001', displayName: 'Modig Bjørk' })
  ...
  it('kaster feil hvis createUser kalles uten nav_ident', async () => {
    await expect(createUser({ navIdent: '' })).rejects.toThrow('nav_ident is required')
  })

  it('kan knytte GitHub-konto til en GitHub-løs bruker i etterkant', async () => {
    await createUser({ navIdent: 'Z990001', displayName: 'Stille Skog' })
    const linked = await upsertUserMapping({ githubUsername: 'stille-skog', navIdent: 'Z990001' })
...
describe('deleteUser — atomisk sletting med rollrevoking', () => {
  ...
  it('soft-sletter bruker, GitHub-kontoer og roller i én transaksjon', async () => {
    ...
    await createUser({ navIdent: 'Z990001', displayName: 'Glad Fjord' })
    await upsertUserMapping({ githubUsername: 'glad-fjord', navIdent: 'Z990001' })
  ...
  it('gjenfinner ikke slettet bruker via getUserMappingByNavIdent', async () => {
    await createUser({ navIdent: 'Z990001' })
    await deleteUser('Z990001', 'Z990002')

    expect(await getUserMappingByNavIdent('Z990001')).toBeNull()
  })

  it('reaktivering via createUser arver ikke revokerte roller', async () => {
  ...
  it('deleteUser via nav_ident sletter GitHub-konto men betar deleteUserMapping for avkoblet konto', async () => {
