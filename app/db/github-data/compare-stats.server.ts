...
export async function saveCompareRawSnapshot(
  owner: string,
  repo: string,
  githubRepoId: number,
  baseSha: string,
  ...
  const result = await pool.query(
    `INSERT INTO github_compare_raw_snapshots
       (github_repo_id, owner, repo, base_sha, head_sha, api_version, api_deprecated_at, api_sunset_at, data)
...
export async function getLatestCompareRawSnapshot(
  ...
  baseSha: string,
  headSha: string,
): Promise<CompareRawSnapshot | null> {
  const result = await pool.query(
    `SELECT id, owner, repo, github_repo_id, base_sha, head_sha, api_version, api_deprecated_at, api_sunset_at, fetched_at, data
