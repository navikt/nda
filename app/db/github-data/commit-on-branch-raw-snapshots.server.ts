...
export async function saveCommitOnBranchRawSnapshot(
  owner: string,
  repo: string,
  githubRepoId: number,
  commitSha: string,
...
export async function getLatestCommitOnBranchRawSnapshot(
  ...
  commitSha: string,
  branch: string,
): Promise<CommitOnBranchRawSnapshot | null> {
  const result = await pool.query(
    `SELECT id, owner, repo, github_repo_id, commit_sha, branch, api_version, api_deprecated_at, api_sunset_at, fetched_at, data
     FROM github_commit_on_branch_raw_snapshots
     WHERE owner = $1 AND repo = $2 AND commit_sha = $3 AND branch = $4
       AND github_repo_id = (
