...
async function fetchChecksForRefs(
  ...
  repo: string,
  ref: string,
): Promise<{ checks_passed: boolean | null; checks: CheckRun[]; rawSnapshot: ChecksSnapshotData }> {
  ...
  if (checkRunsWithAnnotations.length > 0) {
    ...
    for (const { check, annotations } of annotationResults) {
      rawCheckRuns.push({ ...check, annotations })
    }
  }
  ...
  const checks_passed =
    rawCheckRuns.length === 0
      ? null
      : rawCheckRuns.every((check) => check.conclusion === 'success' || check.conclusion === 'skipped')

  return { checks_passed, checks: rawCheckRuns.map(mapRawCheckRunToCheckRun), rawSnapshot }
}
...
export async function getChecksForCommit(
  ...
  repo: string,
  sha: string,
): Promise<{ checks_passed: boolean | null; checks: CheckRun[]; rawSnapshot: ChecksSnapshotData } | null> {
  const client = getGitHubClient()
