...
describe('users.search loader', () => {
  ...
  it('returns search results with Cache-Control: no-store', async () => {
    ...
    expect(searchUsers).toHaveBeenCalledWith('Rask')
  })

  it('returns 500 with Cache-Control: no-store when search fails', async () => {
    vi.mocked(searchUsers).mockRejectedValue(new Error('NOM API error'))
  ...
  it('does not call searchUsers when query is too short', async () => {
