...
describe('parseCheckRunsSnapshot — schema v1 (frozen contract)', () => {
  ...
  it('computes checks_passed=null when a check run is still in progress, instead of treating it as failed', () => {
    const result = parseCheckRunsSnapshot({
      schemaVersion: 1,
      checkRuns: [
        { ...SCHEMA_V1_FIXTURE.checkRuns[0] },
        { ...SCHEMA_V1_FIXTURE.checkRuns[1], status: 'in_progress', conclusion: null },
      ],
    })
  ...
  it('computes checks_passed=null when a check run is queued', () => {
    const result = parseCheckRunsSnapshot({
      schemaVersion: 1,
      checkRuns: [{ ...SCHEMA_V1_FIXTURE.checkRuns[0], status: 'queued', conclusion: null }],
    })
