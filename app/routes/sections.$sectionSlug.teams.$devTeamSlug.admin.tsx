...
import { getOrCreateUserFromNom, upsertUserAndGithubAccount } from '~/db/user-github-lookups.server'
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (intent === 'assign_role') {
    ...
    let knownUser: Awaited<ReturnType<typeof getOrCreateUserFromNom>>
    try {
      knownUser = await getOrCreateUserFromNom(navIdent)
    } catch (err) {
