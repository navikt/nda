...
Kriteriet over («inngang til selve four-eyes-avgjørelsen») dekker ikke alt: enkelte GitHub-kall driver rapporter og visning som man ønsker å kunne endre eller utvide senere uten et nytt, tidssensitivt GitHub-kall. Fire slike kall arkiverer nå det ufiltrerte GitHub-svaret, etter samme mønster som over:
...
- **`lookupLegacyByCommit()`/`lookupLegacyByPR()`** (`app/lib/github/legacy.server.ts`) — den manuelle legacy-godkjenningsflyten henter `getCommit`/`pulls.get`, men lagret tidligere kun utvalgte felt (commit-melding, PR-tittel, mergedBy, reviewers) permanent på `deployments`. `getCommit`-responsen arkiveres nå via samme `archiveCommitRawSnapshot()` som `haveSameCommitTree()` bruker (`github_commit_raw_snapshots`); `pulls.get`-responsen arkiveres i `github_pr_window_raw_snapshots` (se under).
...
Alle fire arkiveringene er best-effort og ikke koblet inn i lesestien: de kallende funksjonene gjør fortsatt alltid et faktisk GitHub-kall (uendret oppførsel), og skriver kun til rådataarkivet som et sideeffekt-kall etter et vellykket API-svar. `github_repo_id` resolves via samme `getRepositoryId()`-cache som de øvrige arkiveringene.

> **Koderef**: `getPullRequestForCommit()` i [`app/lib/github/pr.server.ts`](../app/lib/github/pr.server.ts), `saveCommitAssociatedPrsRawSnapshot()`/`getLatestCommitAssociatedPrsRawSnapshot()` i [`app/db/github-data/commit-associated-prs-raw-snapshots.server.ts`](../app/db/github-data/commit-associated-prs-raw-snapshots.server.ts), `github_commit_associated_prs_raw_snapshots`-tabellen ([`app/db/migrations/1787679200000_add-github-commit-associated-prs-raw-snapshots.sql`](../app/db/migrations/1787679200000_add-github-commit-associated-prs-raw-snapshots.sql)); `lookupLegacyByCommit()`/`lookupLegacyByPR()` i [`app/lib/github/legacy.server.ts`](../app/lib/github/legacy.server.ts); `getSingleCommitMessage()` i [`app/lib/github/git.server.ts`](../app/lib/github/git.server.ts); `getMergedPullRequestsInWindow()` i [`app/lib/github/pr/merged-window.server.ts`](../app/lib/github/pr/merged-window.server.ts), `savePrWindowRawSnapshot()`/`getLatestPrWindowRawSnapshot()` i [`app/db/github-data/pr-window-raw-snapshots.server.ts`](../app/db/github-data/pr-window-raw-snapshots.server.ts), `github_pr_window_raw_snapshots`-tabellen ([`app/db/migrations/1787679300000_add-github-pr-window-raw-snapshots.sql`](../app/db/migrations/1787679300000_add-github-pr-window-raw-snapshots.sql)).
